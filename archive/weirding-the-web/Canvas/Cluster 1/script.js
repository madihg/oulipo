...
document.addEventListener("DOMContentLoaded", () => {
  ...
  let startX = 0,
    startY = 0; // Starting position of the mouse relative to the page
  ...
  document.addEventListener("mousemove", function (e) {
    if (selectedImage) {
      ...
      selectedImage.style.left =
        e.clientX - startX - imageContainer.offsetLeft + "px";
      selectedImage.style.top =
        e.clientY - startY - imageContainer.offsetTop + "px";
    }
  });
...
document.getElementById("openPageBtn").addEventListener("click", function () {
  ...
  window.open(
    "https://www.oulipo.xyz/Canvas/Cluster 1/qrcode.html",
    "_blank",
    "width=600,height=800,right=100,top=200,toolbar=no,scrollbars=no,resizable=yes",
  );

  // Opens the second link in a new window or tab
  window.open(
    "https://www.oulipo.xyz/where-is-home-for-you/index.html",
    "_blank",
    "width=600,height=400,left=250,top=250,toolbar=no,scrollbars=no,resizable=yes",
  );
});
